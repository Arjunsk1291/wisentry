...
## 2026-09-23 — Round 2: research-based CSI analytics, 3D room map, denser dashboard
**Type:** success
**Phase:** 2–3 (simulation only)
**What happened:** Read published WiFi CSI sensing work and applied the magnitude-only methods that fit a single-antenna ESP32: CARM (Wang et al., MobiCom 2015; PCA across subcarrier streams, drop the first component, STFT energy per frequency as a movement-speed proxy) and respiration-rate methods reviewed in VitalCSI (PMC12788229: Hampel + band-pass, breathing-to-noise-ratio band selection, FFT peak). New backend/csi_analytics.py: respiration rate (0.1–0.6 Hz band-pass, first-difference whitening, BNR-selected bands, zero-padded FFT, only over the trailing still segment, cleanest-link fusion), CARM-style motion spectrogram + speed index, per-band attenuation heatmap, per-link activity. Histories are resampled on the sender's timestamps (handles jitter, drops, out-of-order frames and the ESP32 32-bit µs wrap). Dashboard: 3D room with walls, receivers, TX→RX links that glow with link activity and the person's skeleton at the estimated position; new respiration, spectrogram and attenuation panels. Refresh 200 → 400 ms (two WebGL scenes).
